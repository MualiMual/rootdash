...
models = {
    ...
    "people": {
        "model_path": "test_data/ssd_mobilenet_v2_coco_quant_postprocess_edgetpu.tflite",  # Person detection model
        "label_path": "test_data/coco_labels.txt"  # COCO labels file
    }
}
...
def get_frame():
    ...
    # Process frame with the "people" model
    if "people" in interpreters:
        interpreter = interpreters["people"]
        ...
        for i in range(num_detections):
            if scores[0][i] > 0.3:  # Confidence threshold (adjust as needed)
                class_id = int(classes[0][i])
                if class_id == 0:  # COCO label 0 is "person"
                    person_count += 1
    ...
    # Process frame with other models (plants, bugs, etc.)
    for category, interpreter in interpreters.items():
        if category == "people":
            continue  # Skip people model, already processed
